...
const apiUrl = `http://localhost:8080/ppm/ViewAllPlanters`;
...
function displayPlanterDatas(data){
    let pr=document.getElementById("planterRow");
    pr.innerHTML="";

    data.forEach(element => {
        pr.innerHTML+=`
            <tr>
               <td>${element.planterHeight}</td>
               <td>${element.planterColor}</td>
               <td>${element.planterShape}</td>
               <td>${element.planterCapacity}</td>
               <td>${element.planterCost}</td> 
